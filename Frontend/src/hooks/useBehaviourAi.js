/* ================================================================
   ...
   /* ================================================================
      HOOK
   ================================================================ */
   export function useBehaviourAI({ onDetect } = {}) {
     ...
     const poll = useCallback(() => {
       const results = score();
       const { adhd, dyslexia, motor } = results;
   
       /* Update reactive scores state so consumers can read it directly */
       setScores(results);
   
       if (!latched.current.dyslexia && dyslexia.probability >= CFG.THRESHOLD.dyslexia) {
         latched.current.dyslexia = true;
         onDetectRef.current?.("dyslexia", dyslexia.probability, dyslexia.signals);
       }
       if (!latched.current.adhd && adhd.probability >= CFG.THRESHOLD.adhd) {
         latched.current.adhd = true;
         onDetectRef.current?.("adhd", adhd.probability, adhd.signals);
       }
       if (!latched.current.motor && motor.probability >= CFG.THRESHOLD.motor) {
         latched.current.motor = true;
         onDetectRef.current?.("motor", motor.probability, motor.signals);
       }
     ...
     return { getScores: score, latched: latched.current, scores };
   }