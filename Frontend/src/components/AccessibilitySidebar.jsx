...
function Toggle({ checked, onChange }) {
  return (
    <button
      ...
      className={`relative w-12 h-6 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-transparent ${
        checked
...
function ToolRow({ icon, label, checked, onChange }) {
  return (
    <div
      className={`flex items-center justify-between px-4 py-3.5 rounded-2xl mb-2.5 transition-all duration-300 border ${
        checked
...
export default function AccessibilitySidebar() {
  ...
  const { getScores } = useBehaviourAI({ onDetect: handleDetect });
      ...
      try {
        ...
        if(!aiActive) {
          const top = Object.entries({
            ADHD:res.adhd.probability, Dyslexia:res.dyslexia.probability, Motor:res.motor.probability
          }).sort((a,b)=>b[1]-a[1])[0];
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-1">
          ...
          <div className="flex items-center justify-between px-4 py-3.5 rounded-2xl mb-2.5 bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] transition-all">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-xl text-xs font-black tracking-wide transition-all ${
                narrating
